...
object DurabilityRender: EventHandler<RenderItemOverrideEvent> {
    ...
    object LoadListener: EventHandler<ItemLoadEvent> {
        override fun handle(event: ItemLoadEvent) {
            ...
            val durability = tooltip.asSequence().filter { it.string == "" && it.siblings.isNotEmpty() }
                .map { it.siblings[0] }.filter { it.string == "" && it.siblings.size >= 2 }
                .filter { it.siblings[0].string.contains("Crafted ") }
                .map { it.siblings[1].string }.firstNotNullOfOrNull {
